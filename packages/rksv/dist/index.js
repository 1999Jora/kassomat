...
__export(index_exports, {
  ATrustClient: () => ATrustClient,
  ATrustError: () => ATrustError,
  DEPBuilder: () => DEPBuilder,
  FinanzOnlineClient: () => FinanzOnlineClient,
  FinanzOnlineError: () => FinanzOnlineError,
  FiskaltrustClient: () => FiskaltrustClient,
  FiskaltrustError: () => FiskaltrustError,
  buildDEPExport: () => buildDEPExport,
  buildQRCodeData: () => buildQRCodeData,
  buildSigVorigerBeleg: () => buildSigVorigerBeleg,
  calculateInitialHash: () => calculateInitialHash,
  calculateReceiptHash: () => calculateReceiptHash,
  createClosingReceipt: () => createClosingReceipt,
  createMonthReceipt: () => createMonthReceipt,
  createNullReceipt: () => createNullReceipt,
  createStartReceipt: () => createStartReceipt,
  createTrainingReceipt: () => createTrainingReceipt,
  createYearReceipt: () => createYearReceipt,
  encryptSpecialMarker: () => encryptSpecialMarker,
  encryptUmsatzzaehler: () => encryptUmsatzzaehler,
...
function calculateInitialHash() {
  return (0, import_crypto.createHash)("sha256").update("", "utf8").digest("hex");
}
...
function isRetriable(error) {
  if (!import_axios.default.isAxiosError(error)) return false;
...
function buildSigVorigerBeleg(prevSignature) {
  ...
  const hash = (0, import_crypto2.createHash)("sha256").update(input, "base64").digest();
...
function encryptUmsatzzaehler(sumCents, kassenId, belegnummer, aesKey) {
  ...
  const cipher = (0, import_crypto3.createCipheriv)("aes-256-ctr", aesKey, iv);
...
function buildUmsatzzaehlerIV(kassenId, belegnummer) {
  const hash = (0, import_crypto3.createHash)("sha256").update(kassenId, "utf8").update(belegnummer, "utf8").digest();
...
var DEPBuilder = class {
  ...
  async exportToFile(path) {
    ...
    await (0, import_promises.writeFile)(path, json, "utf8");
  }
