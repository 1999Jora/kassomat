...
function MyPOSTab({ settings }: { settings: TenantSettings }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    // Only include api/secret key if the user has typed something new
    // (fields are intentionally left blank if already configured)
    if (!mypos?.configured && (!apiKey.trim() || !secretKey.trim())) {
      toast.error('API-Key und Secret-Key sind erforderlich');
      return;
    }

    const myposPayload: Record<string, string | null | undefined> = {
      storeId: storeId.trim(),
      terminalSerial: terminalSerial.trim() || null,
    };

    // Only send keys if the user has entered them
    if (apiKey.trim()) myposPayload['apiKey'] = apiKey.trim();
    if (secretKey.trim()) myposPayload['secretKey'] = secretKey.trim();

    // If already configured and no new keys entered, we need placeholders
    // to avoid the backend nullifying them — send existing markers instead
    // In practice: if not configured yet both are required (validated above)
    // If already configured and keys left blank, skip update of those fields
    if (mypos?.configured && !apiKey.trim()) {
      // Don't send apiKey — backend only updates if present
      delete myposPayload['apiKey'];
    }
    if (mypos?.configured && !secretKey.trim()) {
      delete myposPayload['secretKey'];
    }

    // If neither key is being updated, we still need them in the payload structure
    // Build the final payload based on what's available
    ...
    // If existing config and no new keys entered, we can't send a partial update
    // via the current schema — pass placeholder empty strings to avoid re-encrypt
    // The backend encrypt() only runs when the field is present, so omitting works
    // with our update() logic as long as we don't pass undefined keys.
    // To keep things simple: always require full credentials on first config,
    // allow updating storeId/terminal without re-entering keys by sending
    // the saved (masked) value — but since we don't have plaintext, we skip those fields.
    // The tenant.service.ts only sets encrypted fields when present in the input.
